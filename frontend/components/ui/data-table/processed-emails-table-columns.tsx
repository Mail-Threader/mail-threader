...
interface GetProcessedEmailTableColumnsProps {
	setRowAction: (action: DataTableRowAction<ProcessedData> | null) => void;
	onViewDetails: (email: ProcessedData) => void;
}
...
export function getProcessedEmailTableColumns({
	setRowAction,
	onViewDetails,
}: GetProcessedEmailTableColumnsProps): ColumnDef<ProcessedData>[] {
	return [
		{
			accessorKey: 'subject',
			header: 'Subject',
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				if (!date)
					return (
						<span className="truncate text-right block min-w-[140px]">
							—
						</span>
					);
