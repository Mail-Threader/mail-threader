...
export function PageTransition({ children }: PageTransitionProps) {
  ...
  useEffect(() => {
    ...
    if (curtainTopLeftRef.current && curtainBottomRightRef.current && contentRef.current) {
      // Ensure curtains are reset to cover the screen initially and are visible
      gsap.set(curtainTopLeftRef.current, { x: '0%', y: '0%', opacity: 1 });
      gsap.set(curtainBottomRightRef.current, { x: '0%', y: '0%', opacity: 1 });
      ...
      .to(curtainBottomRightRef.current, {
      ...
      .to(contentRef.current, {
        ...
        duration: 0.4,
        ease: 'power2.out',
      }, "-=0.3"); // Start content fade-in slightly before curtains fully exit
    }
