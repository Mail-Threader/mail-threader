...
from utils import DatabaseManager, load_processed_df
...
class DataPreparation:
    ...
    def split_all_messages(email_text: str):
        ...
        if forward_matches:
            ...
            for i in range(len(forward_matches)):
                ...
                next_forward_start = (
                    forward_matches[i + 1].start() if i + 1 < len(forward_matches) else None
                )
    ...
    def process_all_emails(self, limit: int = None):
    ...
    def load_data(self, limit: int = None) -> pd.DataFrame:
        ...
        """
        df = load_processed_df(self.output_dir, "processed_data_", limit=limit)
        return df

    @staticmethod
    def save_to_db(df: pd.DataFrame, db: DatabaseManager):
        """
        Save a cleaned email dataframe to a database.
        Args:
            df (pd.DataFrame): Cleaned email dataframe.
            db (DatabaseManager): Database manager.
        Returns:
            bool: True if data-frame was successfully saved, False otherwise
        """
        if not df.empty:
            table_name = "processed_emails"
            success_message = f"\n✅ Saved {len(df)} rows to PostgreSQL table: {table_name}"
            res = db.save_df_to_db(
                df=df, table_name=table_name, suc_msg=success_message, if_exists="replace"
            )
            return res
        else:
            return False
