...
class DataPreparation:
	...
	def split_headers_body(email_text):
		...
		# add: if you see the original message or forward message ... split and don't save as body
		return headers, body
	...
	def parse_original_block(original_block):
		...
		original_info = re.split(r"^-{2,}\s*Original Message\s*-{2,}$",
			original_block,
			flags=re.MULTILINE)
	...
	@staticmethod
	def split_all_messages(email_text: str):
		"""
		...
		forward_matches = list(forward_pattern.finditer(
			email_text))  # find all non-overlapping occurrences of a pattern within the string
	...
	def process_all_emails(self, limit: Optional[int] = None):
		...
		total_files = limit or sum(len(files) for _, _, files in os.walk(self.input_dir))
		...
		df["hash"] = df["body"].apply(self.str_hash)
	...
	@staticmethod
	def str_hash(data: str):
		"""
        Hash function for string data.
        """
		return hashlib.sha256(data.encode()).hexdigest()

	def save_to_firestore(self, db: Optional[AsyncClient], df: Optional[pd.DataFrame] = None):
		"""
        Save the processed DataFrame to Firestore.

        Args:
            db (AsyncClient): Firestore database client or None if not initialized.
            df (pd.DataFrame): DataFrame containing processed email data.
        If None, it will attempt to load the data from the output directory.
        If the DataFrame is empty, it will log a warning and return without saving.
        """

		if db is None:
			logger.error("Firestore database client is not initialized. Cannot save data.")
			return

		if df is None or df.empty:
			logger.warning("No data to save to Firestore. DataFrame is empty.")
			return
...
if __name__ == "__main__":
	...
	if db is None:
		...
		exit(1)
	data_prep.save_to_firestore(db, data)
	print("Data preparation completed and saved to Firestore.")
